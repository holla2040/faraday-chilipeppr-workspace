...
cpdefine("inline:com-hollabaugh-faraday-chilipeppr-workspace", ["chilipeppr_ready"], function() {
    return {
        ...
        init: function() {
            ...
            chilipeppr.subscribe("/com-chilipeppr-widget-serialport/recvline",this.statusHandler.bind(this));
        ...
        statusHandler: function(reply){
            ...
            if (reply.dataline[0]=='<'){ // 0 is spindle
                $('#com-hollabaugh-faradaycoilwinder-status').text(reply.dataline);
                var switches = reply.dataline.split(":")[3];
                // console.log(">>>>>>>>>>"+switches+"-----"+this.switchesLast+"))))");
                if ((switches[0]=='1') && (this.switchesLast[0]=='0')) {
                    chilipeppr.publish("/com-chilipeppr-widget-serialport/send","~g91f25g1y200\n");
                }
                if ((switches[0]=='0') && (this.switchesLast[0] == '1')) {
                    chilipeppr.publish("/com-chilipeppr-widget-serialport/send","!\n");
                    chilipeppr.publish("/com-chilipeppr-widget-serialport/send",String.fromCharCode(24));
                    setTimeout(function(){
                        chilipeppr.publish("/com-chilipeppr-widget-serialport/send","$x\n");
                    },1000)
                }
                
                if ((switches[1]=='1') && (this.switchesLast[1]=='0')) {
                    chilipeppr.publish("/com-chilipeppr-widget-serialport/send","~g91f100g1x-100\n");
                }
                if ((switches[1]=='0') && (this.switchesLast[1] == '1')) {
                    chilipeppr.publish("/com-chilipeppr-widget-serialport/send","!\n");
                    chilipeppr.publish("/com-chilipeppr-widget-serialport/send",String.fromCharCode(24));
                    setTimeout(function(){
                        chilipeppr.publish("/com-chilipeppr-widget-serialport/send","$x\n");
                ...
                if ((switches[2]=='1') && (this.switchesLast[2]=='0')) {
                    chilipeppr.publish("/com-chilipeppr-widget-serialport/send","~g91f100g1x100\n");
                }
                if ((switches[2]=='0') && (this.switchesLast[2] == '1')) {
                    chilipeppr.publish("/com-chilipeppr-widget-serialport/send","!\n");
                    chilipeppr.publish("/com-chilipeppr-widget-serialport/send",String.fromCharCode(24));
                    setTimeout(function(){
                        chilipeppr.publish("/com-chilipeppr-widget-serialport/send","$x\n");
                ...
                this.switchesLast = switches;
                
                var positions = reply.dataline.split(":")[2].split(',');
                $('#com-hollabaugh-faradaycoilwinder-x').text(positions[0]);
                $('#com-hollabaugh-faradaycoilwinder-turns').text(positions[1]);
                $('#com-hollabaugh-faradaycoilwinder-jog').text(reply.dataline.split(":")[3].substr(0,3));
            }
        },
        /**
